...
function select($scope, type, val) {
	if ($scope.selected[type])
		$scope.selected[type].selected = false;
	val.selected = true;
	$scope.selected[type] = val;
	if (!val.availability)
		val.availability = "00000000000000000000000000000000000";
}
...
function DataController($scope, Professor, Group, Room) {
	...
	$scope.clearAll = function() {
		$.each($scope.professors, function(index, item) {
			$scope.$root.clear(item);
		});
	...
	$scope.save = function(val) {
		if (!$scope.modalShown) {
			$scope.changed = {
				professors : $.grep($scope.professors, function(value, index) {
					return value.backup;
				}),
				groups : $.grep($scope.groups, function(value, index) {
					return value.backup;
				}),
				rooms : $.grep($scope.rooms, function(value, index) {
					return value.backup;
				}),
			};
			$scope.modalShown = true;
		} else {
			var count = $scope.changed['professors'].length
			...
			$.each($scope.changed['professors'], function(index, value) {
				if (value.backup) {
					$.ajax({
						type : 'PUT',
						url : "/rest/json/profList/availability/put/"
								+ value.id,
						data : JSON.stringify({
							id : value.id,
			...
			$.each($scope.changed['groups'], function(index, value) {
				if (value.backup) {
					$.ajax({
						type : 'PUT',
						url : "/rest/json/studygroup/availability/put/"
								+ value.id,
						data : JSON.stringify({
							id : value.id,
							availability : value.availability
